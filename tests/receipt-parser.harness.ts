...
const cases: Case[] = [
  ...
  { input: 'מלפפון 2.90', expectItems: true, expectedName: 'מלפפון', expectedPrice: 2.9 },
  { input: 'BREAD 7.50', expectItems: true, expectedName: 'BREAD', expectedPrice: 7.5 },
  {
    input: 'שוקו x2 5.00',
    expectItems: true,
    expectedName: 'שוקו',
  ...
  // --- real-receipt cases ---
  {
    input: 'שניצל עוף פרימיום 34.37',
    expectItems: true,
    expectedName: 'שניצל עוף פרימיום',
    expectedPrice: 34.37,
  },
  ...
  { input: 'mw 12.90 ביצים', expectItems: true, expectedName: 'ביצים', expectedPrice: 12.9 },
  { input: '₪ 24.90 שמן זית', expectItems: true, expectedName: 'שמן זית', expectedPrice: 24.9 },
  ...
  { input: 'ביצים 12.90₪', expectItems: true, expectedName: 'ביצים', expectedPrice: 12.9 }, // trailing ₪
  ...
  {
    input: 'שניצל עוף 9876543210 34.37',
    expectItems: true,
    expectedName: 'שניצל עוף',
    expectedPrice: 34.37,
  },
  ...
  {
    input: 'בקר טחון 56.90 * 0.604',
    expectItems: true,
    expectedName: 'בקר טחון',
    expectedPrice: 34.37,
    expectedQty: 0.604,
  },
  ...
  // but their names never merge (page 1 name must not contaminate page 2 item).
  {
    ...
    expectItems: true,
    expectedItemCount: 2,
    expectedPrice: 7.9, // first item is "חציל 7.90"
  },
  ...
  {
    input:
      'המוצרים בחנות זו עברו בדיקת איכות קפדנית ואנו מבטיחים שביעות רצון מלאה ללקוחות שלנו תמיד וכל הזמן 12.90',
    expectItems: false,
  },
  ...
  { input: 'ביצים 12.90\n-2.00', expectItems: true, expectedName: 'ביצים', expectedPrice: 10.9 },
  // Trailing-minus discount line applied to previous item.
  { input: 'לחם קל 7.50\n2.00-', expectItems: true, expectedName: 'לחם קל', expectedPrice: 5.5 },
  ...
  // "*ק/..." prefix must not prevent the discount from being applied to the previous item.
  {
    ...
    expectItems: true,
    expectedName: 'אצבעות שוקולד חלב',
    expectedPrice: 2.9,
  },
  ...
  // ₪13.20 / 6.60 = 2 → quantity=2; promo line "מבצע" is excluded from name.
  {
    input: "₪13.20\n6.60\nליח'\nחומוס גרגירים שלם\n7290120860155\nמבצע 2ב12 ₪ -1.20",
    expectItems: true,
    expectedName: 'חומוס גרגירים שלם',
    expectedPrice: 13.2,
    expectedQty: 2,
  },
  ...
  // cleanName strips bare numbers, so "55" is removed → "ציטוס פופס גר".
  {
    input: "₪3.90\n3.90\n1\nליח'\nציטוס פופס 55 גר\n7290000000001",
    expectItems: true,
    expectedNameStartsWith: 'ציטוס פופס',
    expectedPrice: 3.9,
    expectedQty: 1,
  },
...
for (const c of cases) {
  ...
  if (c.expectItems && items[0]) {
    ...
    if (
      c.expectedNameStartsWith !== undefined &&
      !items[0].name.startsWith(c.expectedNameStartsWith)
    ) {
      errors.push(
        `expected name to start with "${c.expectedNameStartsWith}", got "${items[0].name}"`,
      );
    }
