...
export class ShoppingListController {
    ...
    deleteItem = catchAsync(async (req: Request, res: Response) => {
        ...
        const updated = await this.service.deleteItem(req.user.id, req.params.listId, req.params.itemId);
        res.status(200).json(updated);
    });
    ...
    toggleItemPurchased = catchAsync(async (req: Request, res: Response) => {
        ...
        const updated = await this.service.togglePurchased(req.user.id, req.params.listId, req.params.itemId);
        res.status(200).json(updated);
    });
}
